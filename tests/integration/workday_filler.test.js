...
// Example: WORKDAY_TEST_URL="https://<company>.wd5.myworkdayjobs.com/<job>" npm test
const TEST_URL = process.env.WORKDAY_TEST_URL;

jest.setTimeout(120000);

// IMPORTANT: mock AI/matcher so tests don’t hit OpenAI or backend.
// If your puppeteer_filler calls into match_question through some other module,
...
run("Workday Application Filler (integration)", () => {
  ...
  it("fills application in dry-run mode without submitting", async () => {
    await expect(
      fillApplication(TEST_URL, { dryRun: true })
